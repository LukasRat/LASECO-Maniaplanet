...
class PluginLazyButtons extends Plugin {
	...
	*/
	...
	public function loadTemplate ($state) {
		...
		$xml .= '<quad pos="0 0" z-index="0.001" size="'. (($command_amount * 10.75) + 25) .' 6.7" bgcolor="222233BB" bgcolorfocus="222233BB" ScriptEvents="1"/>';
		$xml .= '<quad pos="'. (($command_amount * 10.75) + 16.5) .' -0.2" z-index="0.002" size="6 6" image="https://mtv-host.com/trackmania/MLServer/ML_Clan_Logo_red.png"/>';
		...
		foreach ($this->config['COMMANDS'][0]['ENTRY'] as $item) {
			$xml .= '<quad pos="'. ($col + $offset) .' -0.9375" z-index="0.2" size="10 4.6875" action="PluginLazyButtons?Action=Button&MessageId='. $command_count .'" bgcolor="222233BB" bgcolorfocus="222233BB" id="LazyButtons'. $command_count .'" ScriptEvents="1"/>';
