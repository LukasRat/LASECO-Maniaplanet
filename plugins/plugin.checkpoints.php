...
EOL;
		...
		if ($this->config['TIME_DIFF_WIDGET'][0]['BACKGROUND_COLOR'][0] != '') {
			$xml .= '<quad pos="0 0" z-index="0.01" size="40 7.5" bgcolor="222233BB'. $this->config['TIME_DIFF_WIDGET'][0]['BACKGROUND_COLOR'][0] .'"/>';
		}
		else {
	...
	*/
	...
	public function buildCounterWidget ($checkpoint = -1, $login = false) {
		...
		if ($this->config['COUNT_WIDGET'][0]['BACKGROUND_COLOR'][0] != '') {
			$xml .= '<quad pos="0 0" z-index="0.001" size="40 7.5" bgcolor="222233BB'. $this->config['COUNT_WIDGET'][0]['BACKGROUND_COLOR'][0] .'"/>';
		}
		else {
		...
		$xml .= '<label pos="20 -1.21875" z-index="0.01" halign="center" textsize="1" scale="0.6" textcolor="FFF" text="" id="CheckpointLine1"/>';
